...
<style>
	...
	<div class="frame" id="frameWelcomeTop">		
		<div class="container">		
		    <div class="jumbotron">
		    	<h1 id="hello,-world!">Ecran de la file d'attente</h1>
		    	<br />
		    	<p>Cet affichage est destiné aux patients du service.</p>
		 		<br /><br /><br /><br />
				<form:form id="formWelcome" method="post" action="/easyessoft/ihm/waitingQueue/screenWaiting" commandName="waitingQueue" >	
					<fieldset>		
							<div class="form-group">
								<p>
									<label>Type de Service</label>
			
									<form:select id="listServiceType" class="form-control" path="idService">
										<form:option value='0' label="<Selectionnez>" />
										<form:option value="1" label="Urgence" />
									</form:select>
								</p>
								<p>
									...
									<form:select disabled="true" id="listOrganization" class="form-control" path="idOrganization">
										<form:option value='0' label='<Selectionnez>' />
											<c:forEach items="${listOrganization}" var="organization">
												<form:option class="serviceType${organization.orgaType.id}" value="${organization.id}">${organization.name}</form:option>
											</c:forEach>
									</form:select>
								</p>
							</div>
						<br />
						<a class="btn btn-primary btn-lg center-block" href="#" id="showButton">Afficher</a>
					</fieldset>
				</form:form>
 	...
 	<div class="frame" id="frameWelcomeBottom">		
  		<div class="container">
		
		<br />
			
	<div>
		<h1>Hopital Beaujon - Service d'Urgence</h1>
	</div>
	...
	<form:form id="form" method="post" action="/easyessoft/ihm/ref/staffAdmin/member/createAction" commandName="staffMember2" >	
		...
		<fieldset>
			...
			<legend style="vertical-align: middle;">Prochains passages <span style="float: right;"><img src="http://icons.iconarchive.com/icons/alecive/flatwoken/128/Apps-Clock-icon.png" style="float: left; width: 28px; height: 28px; margin-right: 10px;"/><div id="clock" style="float: left; width: 92px;"></div></span></legend>
