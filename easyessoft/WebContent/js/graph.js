...
var Log = {
	...
	write : function(text) {
		...
		this.elem.style.left = ((1100 - this.elem.offsetWidth) / 2) + 'px';
	}
};
	...
	fd = new $jit.ForceDirected(
			{
				Tips : {
					...
					onShow : function(tip, node) {
						var txt = '<h5>' + node.name + '</h5><b>Interaction(s)</b> :<ul>';
						node.eachAdjacency(function(adj) {
							txt = txt + '<li>' + adj.nodeTo.name + '</li>';
						});
						txt = txt + '</ul>'
						tip.innerHTML = txt;
				...
				// Number of iterations for the FD algorithm00
				iterations : 500,
				// Edge length
				levelDistance : 120,
